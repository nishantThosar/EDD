...
/*--------------------------------------FUNC PROTOTYPES------------------------------------------------*/
static int __init my_mod_entry();
static void __exit my_mod_exit();
static int thread_func(void *pv);
...
struct thread_info_driver info[MAX_THREADS] = {0}; // array for struct with name info.
...
MODULE_PARM_DESC(MAX_THREADS, "MAX number of threads");
...
static int __init my_mod_init(void)
{
	...
	if (MAX_THREADS == 0)
	{
		pr_emerg("Threads cannot be zero\n");
		return -1;
	}
	else
