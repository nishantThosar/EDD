...
/*The func prototypes are static means no other module can access these modules*/
// function prototypes
static int __init my_mod_init(void);
static int my_open(struct inode *inode, struct file *file); //open the device
static ssize_t my_read(struct file *file, char __user *buf, size_t len, loff_t *off); reading from func
static ssize_t my_write(struct file *file, const char __user *buf, size_t len, loff_t *off); // writing to the file
static int my_release(struct inode *inode, struct file *file); // closing (releasing)
...
// create a fops struct(file oeration structure)
